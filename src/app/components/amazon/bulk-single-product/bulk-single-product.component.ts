...
export class BulkSingleProductComponent implements OnInit {
  ...
  getPromptSection() {
    this.dbService.get('/prompt_section_bulk_product').subscribe((response) => {
  ...
  getShortcodes() {
    this.dbService.get('/shortcodes').subscribe((response) => {
      this.shortcodes = response.filter((item: any) => item.section === 'bulk_product');
    }, (error) => {
      console.log(error);
    });
  }
  ...
  saveSettings() {
    ...
    this.dbService.put('/prompt_section_bulk_product', prompt_settings).subscribe((response) => {
  ...
  addShortcode() {
    this.saveShortcode();
    const newShortcode = { code: '', position: '', section: 'bulk_product' };
